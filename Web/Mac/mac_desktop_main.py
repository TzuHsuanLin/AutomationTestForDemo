...
class SuperTaste(unittest.TestCase):
    ...
    def test_memberCenterLoginFB(self):
        ...
        try:
            ...
            self.driver.find_element_by_id('pass').send_keys('tvbs')
            self.driver.find_element_by_id('loginbutton').click()
            WebDriverWait(self.driver, 10).until(lambda x: x.find_element_by_class_name('layerConfirm'))
            self.driver.find_element_by_class_name('layerConfirm').click()
            sleep(2)
            self.driver.find_element_by_class_name('fb_btn').click()
    ...
    def test_index(self):
        ...
        try:
            ...
            # 同意cookie
            WebDriverWait(self.driver, 20).until(lambda x: x.find_element_by_class_name('btn'))
        ...
        try:
            ...
            for i in range(txt_count - 1):
                ...
                if self.driver.find_elements_by_class_name('txt')[i].text == backend_title:
                    ...
                    sleep(3)
                    all_p = self.driver.find_elements_by_xpath('//p')  # 列表存取頁面上所有p tag元素
                    text_list = []
                    for p in all_p:
        ...
        try:
            ...
            if int(self.driver.find_elements_by_class_name('num')[0].text) > 6:
                ...
                WebDriverWait(self.driver, 20).until(lambda x: x.find_element_by_class_name(
                    'supertaste_article').find_element_by_class_name('list').find_elements_by_class_name(
                    'overlay-color')[6])
            ...
            if int(self.driver.find_elements_by_class_name('num')[1].text) > 6:
                ...
                WebDriverWait(self.driver, 20).until(lambda x: x.find_element_by_class_name(
                    'supertaste_store').find_element_by_class_name('list').find_elements_by_class_name('overlay-color')[
                    6])
            ...
            if int(self.driver.find_elements_by_class_name('num')[2].text) > 6:
                self.driver.find_element_by_class_name('supertaste_commodity').find_element_by_class_name(
                    'more2').click()
                print('相關商品More按鈕正常')
                WebDriverWait(self.driver, 20).until(lambda x: x.find_element_by_class_name('act'))
                focus_text = self.driver.find_element_by_class_name('act').text
...
if __name__ == '__main__':
    ...
    # suite.addTest(SuperTaste('test_memberCenterRegisterFB'))  # 註冊FB會員(X)
